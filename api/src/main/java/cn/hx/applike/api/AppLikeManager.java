...
import cn.hx.applike.api.utils.ClassUtils;

public class AppLikeManager {
    ...
    private static void scanClassFile(Context context) {
        try {
            Set<String> set = ClassUtils.getFileNameByPackageName(context, AppLikeConfig.PROXY_CLASS_PACKAGE_NAME);
            if (set != null) {
                for (String className : set) {
                    if (DEBUG) {
                        Log.d("AppLike", className);
                    }
                    try {
